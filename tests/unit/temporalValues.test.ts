...
Deno.test("parseInstant - accepts PostgreSQL's default output verbatim", () => {
	...
	const fromPostgres = Either.getOrThrow(parseInstant("2026-08-10 19:18:27.361+00"));
	const fromIso = Either.getOrThrow(parseInstant("2026-08-10T19:18:27.361+00:00"));
...
Deno.test("parseInstant - a non-UTC offset resolves to the instant it names", () => {
	const instant = Either.getOrThrow(parseInstant("2026-08-10 19:18:27+05:30"));
...
Deno.test("parseInstant - sub-minute offsets survive", () => {
	...
	const instant = Either.getOrThrow(parseInstant("1883-11-18 12:00:00-00:53:28"));
...
Deno.test("parseInstant - a BC date raises", () => {
	assertEitherFailure(() => parseInstant("0044-03-15 00:00:00+00 BC"), UnrepresentableTemporalValueError);
});
...
Deno.test("parseInstant - a zone-less timestamp raises rather than being read as UTC", () => {
	...
	assertEitherFailure(() => parseInstant("2026-08-10 19:18:27.361"), UnrepresentableTemporalValueError);
});

Deno.test("parsePlainDateTime - keeps wall-clock time and adds no offset", () => {
	const plain = Either.getOrThrow(parsePlainDateTime("2026-08-10 19:18:27.361"));
...
Deno.test("parsePlainDate - a bare calendar day", () => {
	assertEquals(Either.getOrThrow(parsePlainDate("2026-08-10")).toString(), "2026-08-10");
});

Deno.test("parsePlainDate - rejects a value that is not a date", () => {
	assertEitherFailure(() => parsePlainDate("not-a-date"), UnrepresentableTemporalValueError);
});

Deno.test("isTemporalValue - recognizes Temporal values and nothing else", () => {
	assert(Either.getOrThrow(isTemporalValue(Temporal.Instant.from("2026-08-10T00:00:00Z"))));
	assert(Either.getOrThrow(isTemporalValue(Temporal.PlainDate.from("2026-08-10"))));
	assert(Either.getOrThrow(isTemporalValue(Temporal.PlainDateTime.from("2026-08-10T00:00:00"))));
	assert(Either.getOrThrow(isTemporalValue(Temporal.ZonedDateTime.from("2026-08-10T00:00:00[UTC]"))));
	assert(Either.getOrThrow(isTemporalValue(Temporal.Duration.from("PT1H"))));
	...
	for (const other of [null, undefined, 1, "2026-08-10T00:00:00Z", true, {}, [], new Date()]) {
		assert(!Either.getOrThrow(isTemporalValue(other)), `${String(other)} is not a Temporal value`);
	}
});

Deno.test("temporalToPostgres - renders Temporal values to text and leaves the rest alone", () => {
	assertEquals(
		Either.getOrThrow(temporalToPostgres(Temporal.Instant.from("2026-08-10T19:18:27.361Z"))),
		"2026-08-10T19:18:27.361Z",
	);
	assertEquals(Either.getOrThrow(temporalToPostgres(Temporal.PlainDate.from("2026-08-10"))), "2026-08-10");

	const date = new Date(0);
	assertEquals(
		Either.getOrThrow(temporalToPostgres(date)),
		date,
		"a Date must reach pg untouched — pg knows how to serialize it",
	);
	assertEquals(Either.getOrThrow(temporalToPostgres(null)), null);
	assertEquals(Either.getOrThrow(temporalToPostgres("already text")), "already text");
	assertEquals(Either.getOrThrow(temporalToPostgres(42)), 42);
});
...
Deno.test("temporalToPostgres - walks arrays, because `= ANY($1)` is one parameter holding many values", () => {
	...
	// Postgres rejects.
	assertEquals(
		Either.getOrThrow(temporalToPostgres([
	...
	assertEquals(
		Either.getOrThrow(temporalToPostgres(["plain", Temporal.PlainDate.from("2026-08-10"), 7, null])),
		["plain", "2026-08-10", 7, null],
	);
	assertEquals(
		Either.getOrThrow(temporalToPostgres([[Temporal.PlainDate.from("2026-08-10")]])),
		[["2026-08-10"]],
	);
	// An array with nothing to render comes back with the same values.
	assertEquals(Either.getOrThrow(temporalToPostgres([1, "two", null])), [1, "two", null]);
});
...
Deno.test("temporalToPostgres - a ZonedDateTime loses its IANA annotation, which Postgres rejects", () => {
	...
	const rendered = Either.getOrThrow(temporalToPostgres(zoned));
